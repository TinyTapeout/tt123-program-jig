...
Sourced from https://forum.micropython.org/viewtopic.php?f=16&t=3899
"""
...
class W25QFlash(object):
    ...
    @chip_select.setter
    def chip_select(self, setTo:bool):
        # print(f'Select CHIP {setTo}')
        if setTo == self._chipselect:
            return
            
        self._chipselect = setTo
        
        if setTo:
            self.cs(0)
            if self.passthrough_enable:
                self.spi.write(self.HKSPI_PASSTHROUGH)
        else:
            self.cs(1)
    ...
    def identify(self) -> None:
        ...
        if mf != 0xEF or mem_type not in [0x40, 0x60, 0x70]:
            ...
            print(f"Warning manufacturer ({hex(mf)}) or memory type ({hex(mem_type)}) not tested.")
    ...
    def writesector(self, sector_nr: int, buf: list) -> None:
        """
          Write a whole sector (4kb, 8 blocks), which will 
          be erased first.
        """
        assert len(buf) == self.SECTOR_SIZE, \
            "invalid sector length: {}".format(len(buf))
        
        sector_addr = sector_nr * self.SECTOR_SIZE 
        
        self._sector_erase(addr=sector_addr)
        
        self._write(buf=buf, addr=sector_addr)
